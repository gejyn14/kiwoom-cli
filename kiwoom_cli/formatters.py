...
_ABS_FIELDS = frozenset({
    ...
    # WebSocket 0D(주식호가잔량) 10단계 호가 *가격*: 41~50 매도호가, 51~60 매수호가.
    # 스펙이 이 20개에만 "부호가 포함된 숫자"를 명시하고, Response Example도
    # 매도호가1='-20800' / 매도호가2='+20900'처럼 오름차순 가격에 부호가 섞여
    # 나온다 — 즉 +/-는 직전대비 방향지시자이지 값의 부호가 아니다.
    # 대응 수량 필드(61~80)는 여기가 아니라 _SIGNED_FIELDS에 있다(사유는 그쪽 주석).
    *(str(n) for n in range(41, 61)),
...
_SIGNED_FIELDS = frozenset({
    ...
    # WebSocket 0D 10단계 호가 *수량*: 61~70 매도호가수량, 71~80 매수호가수량.
    # ABS가 아니라 SIGNED인 이유: 스펙은 가격(41~60)에만 "부호가 포함된 숫자"를
    # 달아두고 이 20개에는 "단위: 1주"만 적는다. 실제 데이터에서 두 분류는
    # 동작이 같지만(부호가 안 붙으므로), 스펙에 없는 부호가 언젠가 나타났을 때
    # ABS는 abs()로 정보를 조용히 버리고 *_direction 키 20개를 만들어내는 반면
    # SIGNED는 값을 그대로 보존해 이상을 드러낸다. 수량은 방향지시자를 갖는
    # 필드가 아니므로 방향 동반 키 자체가 오해를 부른다.
    *(str(n) for n in range(61, 81)),
