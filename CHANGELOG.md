...
코드, 실시간 호가 필드 정규화, 캐시 신선도, 주문 입력값 하한, `--exchange`
일괄 정비를 다뤘습니다. 이번 릴리스는 breaking 변경이 많고, 그중 상당수는
커밋 제목에 `!`가 붙어 있지 않습니다. 아래 Breaking 절은 표시 여부와 무관하게
동작이 바뀌는 것을 전부 적었습니다.

`kiwoom stream *`은 **인증 실패를 한 번도 감지하지 못하고 있었습니다.**
`streaming.py`가 LOGIN 응답에서 `code`를 읽는데 서버가 보내는 키는
`return_code`입니다. 실패한 인증에도 "인증 성공"을 출력하고 그대로 진행했고,
구독 없는 소켓이 마감 시각까지(마감 조건이 없으면 영원히) 매달렸습니다.
추측이 아니라 무효 토큰으로 mock·prod 양쪽 WebSocket에 LOGIN만 보내 raw
프레임을 캡처해 확인했습니다. 이때 **prod는 LOGIN 응답 뒤에 SYSTEM 프레임을
보내고 mock은 보내지 않는다**는 사실도 함께 드러났습니다. 프레임 순번으로
ack를 집던 `watch`는 그 SYSTEM을 인증 응답으로 오독해 사유가 공란인
"인증 실패"를 찍고 exit 0으로 끝나고 있었습니다. 핸드셰이크를 순번이 아니라
프레임 종류로 읽도록 바꾸고, 실패 경로의 종료 코드를 `stream`과 `watch`가
같은 규칙(인증 3, 그 외 실패 2)으로 쓰도록 맞췄습니다.

`stream orderbook`(0D)은 처음부터 끝까지 정규화되지 않고 있었습니다. 호가시각
필드가 시각 ID로 등록돼 있지 않아 모든 0D 이벤트의 `ts`가 `null`이었고,
10단계 호가 필드 41~80은 어디에도 매핑돼 있지 않아 숫자 ID 문자열 그대로
샜습니다. 숫자 분류에도 없어서 매도호가가 `"-20800"`처럼 방향지시자 부호를
단 채 음수 가격으로 보였습니다.

`market`의 `--exchange` 30곳이 통합(3)을 받게 되고 **기본값이 KRX에서 all로
바뀝니다.** 이번 릴리스에서 가장 눈에 띄는 동작 변경입니다. 받아들이는 값
집합은 넓어지기만 했지만(23곳이 `{KRX, NXT}` → `{KRX, NXT, all}`, 나머지
7곳은 원래부터 3값), **기본값이 바뀌었으므로 옵션 없이 부르던 호출의 결과가
달라집니다.** 두 사실은 별개이고, 후자 때문에 breaking입니다.

캐시 두 곳이 신선도를 주장할 근거 없이 영구화되고 있었습니다. 미국 거래소
캐시는 파일이 하나뿐이라 모의투자와 실거래가 같은 항목을 공유했습니다 —
모의에서 학습한 거래소가 실거래 주문의 `stex_tp`로 그대로 나갈 수 있었습니다.
종목 리스트 캐시는 `fetched_at`을 기록만 하고 읽는 곳이 없어서, 신규 상장·
폐지가 `stock sync`를 손으로 돌리기 전까지 반영되지 않았습니다.

주문 입력값에는 하한이 없었습니다. base에서 실제로 측정한 결과
`ord_qty:"0"`, `ord_uv:"-70000"`, `cncl_qty:"-5"`가 그대로 전송됐고, 미국
경로는 `ord_uv:"nan"`까지 보냈습니다. 국내 NaN/Inf는 `int()` 변환에서
envelope 없이 죽었습니다. 근거는 키움이 배포하는 kwcli 0.1.1의
`maps/arguments.csv`입니다(주문·정정 수량 `positive_int_string`, 취소 수량
`nonnegative_int_string`, 가격 `^\d+$`). 정정 수량 `0`(잔량 전부 정정)은 스펙
비고에 명시된 관용구라 그대로 살렸습니다.

**Fixed**

- **`kiwoom stream *`이 WebSocket 인증 실패를 감지하지 못했습니다.** LOGIN
  응답의 키는 `return_code`/`return_msg`인데 `code`/`message`를 읽고 있었습니다.
  `code`/`message`는 실재하지만 **SYSTEM 프레임의 키**이고, SYSTEM의 키 이름을
  LOGIN에 잘못 적용한 것이 버그의 기원입니다. 그래서 "양쪽 키를 다 허용"하는
  폴백은 넣지 않았습니다 — SYSTEM이 ack 자리에 오면 같은 버그가 재현됩니다.
- **`watch`가 핸드셰이크 프레임을 순번으로 집었습니다.** prod가 LOGIN 응답 뒤에
  보내는 SYSTEM 프레임을 ack로 오독해 `return_code` 부재 → 사유 공란
  "인증 실패" → exit 0으로 끝났습니다. mock만 관측해서는 절대 드러나지 않는
  경로입니다. 핸드셰이크를 프레임 종류로 판정하는 `recv_ack()`로 바꾸고,
  PING 키프얼라이브는 에코하고 SYSTEM은 표시한 뒤 `return_code`를 실은
  프레임까지 읽도록 했습니다. ack를 기다리는 동안 도착한 REAL 프레임을 조용히
  버리던 문제도 함께 고쳤습니다(버퍼링 후 수신 루프가 소비합니다) — 서버가 틱을
  밀기 시작하는 구간이 정확히 그 창입니다.
- **`stream orderbook`(0D)의 `ts`가 항상 `null`이었고 호가 필드가 숫자 ID로
  샜습니다.** 필드 매핑은 워크북 시트 `주식호가잔량(0D)`과 kwcli 0.1.1이 동봉한
  `kiwoom_api_spec.json`을 대조해 확정했고, 두 소스가 필드별로 일치합니다.
  41~50 매도호가, 51~60 매수호가, 61~70 매도호가수량, 71~80 매수호가수량.
  이름이 아니라 값의 순서(매도 오름차순·매수 내림차순, ask1>bid1)로도
  방향을 교차검증했습니다.
- **`A005930` 형태의 국내 종목코드가 미국 엔드포인트로 라우팅됐습니다.** 잔고
  응답은 종목코드에 시장구분 접두사를 붙여 돌려주는데, `is_us_symbol`이 이를
  6자리 숫자가 아니라고 보고 미국으로 판정했습니다. 국내 분기는 `stk_cd`
  필터를 조용히 빼먹어 전 종목을 반환했고, 미국 분기는 `A005930`을 ust21050에
  보냈습니다. `account orders pending` 외에 `pnl today`/`pnl by-period`/
  `orders executed`/`orderable margin-qty`와 `stock`·`order`의 국내 분기 전체가
  같은 뿌리였습니다. 접두사 제거는 **영문 1자 + 숫자 6자리**라는 완성된 모양에만
  적용합니다 — 선행 영문자를 무조건 벗기면 미국 티커가 깨집니다(NVDA → VDA,
  F → 빈 문자열).
- **미국 거래소 캐시를 모의투자와 실거래가 공유했습니다.** 파일이
  `us_exchanges.json` 하나뿐이라 모의에서 학습한 거래소가 실거래 주문의
  `stex_tp`로 나갈 수 있었습니다. 파일명을 도메인별로 분리하고 항목에 시각을
  붙여 24시간이 지나면 무시합니다. 프로필로는 나누지 않습니다 — "NVDA가
  나스닥 상장"은 계좌 속성이 아니라 시장의 사실입니다.
- **종목 리스트 캐시에 만료가 없었습니다.** `_save_stock_cache`가 `fetched_at`을
  기록만 하고 읽는 곳이 없어 `stocks.json`이 영구 캐시였습니다. 필드를 지우는
  대신 판정에 써서 7일 TTL을 적용합니다. 거래소 캐시(24시간)보다 길게 잡은
  것은 상장·폐지가 하루 단위로 흔들리지 않고, 만료 시 재동기화가 ka10099 4회
  호출이라 대화형 `stock search`에서 매일 물리기엔 비싸기 때문입니다.
- **`stock search --market etn`이 구조적으로 항상 빈 결과였습니다.**
  `_sync_stocks`의 루프가 ka10099에 ETN 시장코드 `"60"`을 한 번도 요청하지
  않았습니다. 그런데 `--market etn`은 Choice에 광고돼 있고 필터 맵에도 라벨이
  있어서, 사용자에게는 정상 옵션으로 보이면서 캐시에 ETN이 들어올 경로 자체가
  없었습니다.
- **`history export`가 상위 디렉터리가 없으면 raw traceback으로 죽었습니다.**
  `--out`에 없는 디렉터리를 주면 dest별로 제각각인 예외(sqlite
  `OperationalError`, csv `FileNotFoundError`)가 튀었고 json 모드에서는
  envelope 없이 종료됐습니다. 상위 디렉터리를 만들고, 만들 수 없으면
  `INVALID_INPUT`(exit 1)을 냅니다. 파일을 쓰는 커맨드 중 여기만 빠져 있었습니다.
- **`auth logout`이 폐기하지 않은 키체인 토큰을 지웠습니다.** `load_token`은
  `KIWOOM_TOKEN`을 키체인보다 먼저 돌려주는데, `revoke_token`은 그 env 토큰으로
  폐기 요청을 보낸 뒤 무조건 키체인의 `{profile}:token`을 삭제했습니다 — 방금
  폐기한 것과 다른, 아직 살아 있는 토큰을 영영 폐기할 수 없게 만들었습니다.
  안내 문구("해당 토큰이 계속 사용됩니다")도 사실과 달랐습니다.
- **`migrate_to_profiles`가 부분 실패 시 자격증명을 영구히 고아로 남겼습니다.**
  `save_config()`가 keyring 이전 루프보다 먼저 실행됐는데, `cfg["profiles"]`의
  존재가 곧 마이그레이션 완료 표식입니다. keyring 호출 중 예외가 나면 표식만
  박힌 채로 남아 다음 실행이 가드에 걸리고, bare appkey/secretkey/token이
  이전되지 못한 채 방치됐습니다.
- **`order modify --qty 0`(잔량 전부 정정)이 되살아났습니다.** 이 릴리스에서
  넣은 수량 하한 가드가 kt10002/kt10008/kt50002의 세 정정 경로를 함께 막았던
  것을 같은 릴리스 안에서 고쳤습니다. 스펙 비고에 `'0' 입력 시 잔량 전부 정정`
  이라고 명시돼 있고 v2.12까지 실제로 전송되던 값입니다. 구 워크북에는 이
  비고 칸이 아예 없고 kwcli는 `positive_int_string`으로 0을 거부하는데, 그
  판단 근거의 갈림은 `validate_order_qty` 독스트링에 그대로 적어 뒀습니다.
- `market sector`의 위치인자 9개 metavar을 `SECTOR_CODE`로 통일했습니다
  (`sector index`만 `--sector-code` 옵션이라 도움말에 `SECTOR_CODE`가 나오는데
  나머지는 파라미터 이름 `INDS_CD`가 그대로 노출돼, 한 그룹이 두 어휘를 가르치고
  있었습니다. 표시 이름만 바뀌고 전송 body는 그대로입니다). 그리고 `order` 계열
  `--help` 예제 중 실제로 실행되지 않는 두 건을 고쳤습니다
  (`order validate` 예제가 루트 옵션인 `-f json`을 서브커맨드 뒤에 두고 있어
  그대로 실행하면 exit 1이었습니다).

**Added**

- **`kiwoom config prune-ledger`.** 멱등성 원장은 append-only라 파일도
  `lookup()`의 선형 스캔도 무한히 자랐습니다. 90일 지난 종결 키를 제거합니다.
  트리거를 수동 명령 하나로 둔 이유는 실패의 대가가 비대칭이기 때문입니다 —
  원장이 잘리면 `lookup()`이 `None`을 반환하고 그 결과는 이미 체결됐을 수 있는
  주문의 재전송입니다. `record()`에서 확률적으로 재작성하는 안은 그 위험을
  주문 실행 도중에 무작위로 조용히 터뜨립니다. 판단은 줄이 아니라 키 단위로
  하고(최종 레코드가 inflight면 그 키 전체 보존), 임시 파일 → fsync →
  `os.replace`로 교체해 어느 시점에 죽어도 원장이 잘린 중간 상태로 남지
  않습니다.

**Breaking (커밋 제목에 `!` 표시)**

- **`kiwoom stream *`이 등록 ack 없이 끝나면 exit 0 대신 exit 2를 반환합니다.**
  종전에는 구독이 한 번도 성사되지 않아도 조용히 0으로 끝났습니다. `$?`로
  분기하는 스크립트는 값이 달라집니다. json 모드에서는 종전에 아무것도 찍히지
  않던 자리에 `UPSTREAM_ERROR` envelope 한 줄이 나옵니다. 등록 ack를 받은 뒤의
  정상 종료는 그대로 exit 0입니다.
- **`kiwoom dashboard`의 거래량 상위 목록에 관리종목이 포함됩니다.**
  `market rank volume`의 기본값과 맞추기 위해 ka10030의 `mang_stk_incls`가
  `"1"`(미포함)에서 `"0"`(포함)으로 바뀝니다. 종전에 보이지 않던 종목이
  상위권에 나타날 수 있습니다.
- **`market`의 `--exchange` 30곳에서 기본값이 `KRX`에서 `all`로 바뀝니다.**
  `market rank volume`을 옵션 없이 부르면 지금까지 KRX 단독(`stex_tp=1`)을
  조회했는데 이제 통합(`3`)을 조회합니다. 종전 동작이 필요하면 `--exchange KRX`를
  명시해야 합니다. 받아들이는 값 집합은 23곳이 `{KRX, NXT}` → `{KRX, NXT, all}`로
  넓어지기만 했고 그 자체는 breaking이 아니지만, **기본값이 바뀐 것은 별개의
  사실이고 그쪽이 breaking입니다.** 세 소스가 30/30 일치합니다:
  `docs/미국 REST API 문서.xlsx`(`stex_tp` Required=Y, `3:통합`), kwcli 0.1.1
  번들 `kiwoom_api_spec.json`, 그리고 kwcli의 `maps/arguments.csv`(30곳 전부
  `choices="KRX|NXT|ALL"`, `value_map="KRX=1;NXT=2;ALL=3"`). 셋째는 산문이 아니라
  키움이 배포한 코드의 wire 매핑이라 가장 강한 근거입니다. 함께
  `EXCHANGE_TWO` 상수를 제거했습니다 — market.py 23곳이 유일한 사용처였고
  전부 `EXCHANGE_ALL`로 넘어가 배선이 하나도 남지 않았습니다.
- **`market gold chart-minute`의 `--price-type`과 `market program stock-daily`의
  `--unit`/`--date`가 미지정 시 빈 문자열 대신 키 자체를 생략합니다.** 세 자리
  모두 스펙상 Required=N이고, 미지정은 "빈 값을 명시했다"가 아니라 "보내지
  않는다"입니다. 전송 body가 ka50080은
  `{stk_cd, tic_scope, upd_stkpc_tp:""}` → `{stk_cd, tic_scope}`,
  ka90013은 `{amt_qty_tp:"", stk_cd, date:""}` → `{stk_cd}`로 바뀝니다.
  값을 지정한 호출은 그대로입니다.
- **`market etf returns`의 `--index`(ka40001 `etfobjt_idex_cd`)와
  `market program stock-time`의 `--date`(ka90008 `date`)도 같은 처리를
  받습니다.** 이 둘은 Required=Y인데 빈 문자열을 전송하고 있었습니다. 빈
  문자열도 키 생략도 스펙 위반이지만, 빈 값을 명시적으로 주장하는 대신 키를
  빼면 서버가 자기 필수필드 오류로 답합니다. 두 워크북의 Request 표가
  일치합니다.
- **`history export`의 `ts`에 날짜가 붙고, `data.events`의 의미가 바뀝니다.**
  녹화 NDJSON의 `ts`는 시각만 담고 날짜는 파일명에만 있는데, export가 그 값을
  그대로 써서 중복 제거가 날짜맹목이었습니다 — 여러 날을 한 번에 내보내면
  날짜만 다른 별개 이벤트가 조용히 한 건으로 접혔습니다. sqlite/csv/parquet의
  `ts`가 `"11:00:00+09:00"` → `"2026-07-16T11:00:00+09:00"`으로 바뀌고,
  `data.events`가 수집 건수에서 **실제 삽입된 행수**로 바뀝니다(sqlite
  재-export는 이제 `0`을 보고합니다). **녹화 파일과 `history query`의 `ts`는
  그대로입니다** — NDJSON 한 줄의 모양은 AGENTS.md가 문서화한 계약이고 이미
  기록된 파일과도 섞이므로, 녹화 출력을 바꾸는 쪽이 더 큰 breaking입니다.
  파일명에서 날짜를 읽을 수 없으면 지어내지 않고 녹화 값을 유지합니다.
- **`dashboard`가 보여주는 거래량 상위 목록이 달라집니다.** ka10030의
  `stex_tp`를 `1`에서 `3`으로 바꿔 `market rank volume`과 일치시켰습니다. 위
  `--exchange` 정비가 market 쪽 기본값만 옮기고 이 호출부를 놓쳐 생긴 갈림입니다.
  두 커맨드의 `mang_stk_incls`는 여전히 다릅니다(dashboard `1`, rank `0`).
  이건 D7 이전부터 있던 dashboard의 의도적 선택으로 읽혀 손대지 않았고, 대신
  호출부 주석과 테스트에 명시해 다음 드리프트가 "원래 달랐다" 뒤에 숨지 못하게
  했습니다.

**Breaking (커밋에 `!` 표시가 없지만 동작이 바뀌는 것)**

아래는 커밋 제목이 `fix(...)`로만 돼 있지만 종전에 동작하던 호출의 결과가
바뀌는 것들입니다. 표시가 없다고 영향이 없는 게 아니라서 함께 적습니다.

- **주문 수량·가격에 하한이 생겨, 종전에 전송되던 값이 이제 exit 1입니다.**
  수량 `<= 0`, 음수 가격, NaN/Inf가 여기 해당합니다. 실제로 나가던 값은
  `ord_qty:"0"`, `ord_uv:"-70000"`, `cncl_qty:"-5"`, 환전의
  `fc_exmn_amt:"-500000"`, 그리고 미국 경로의 `ord_uv:"nan"`입니다. 취소의
  `--qty 0`(전량취소)과 정정의 `--qty 0`(잔량 전부 정정)은 문서화된 계약이라
  그대로 동작합니다. 미국 정정(ust20002)은 요청 스펙에 수량 필드가 아예 없어
  **0이 아닌 수량을 거부합니다** — 종전에는 수량을 조용히 버리고 stderr 경고만
  냈는데, 그 경고는 수량 미지정 시에도 무조건 떴고 json 소비자에게는 보이지
  않아 "수량 5로 정정 성공"으로 읽혔습니다.
- **`order validate`의 판정이 수량 0과 음수 가격에서 뒤집힙니다.** 종전에는
  `valid: true`를 답하는데 실주문은 거부했습니다 — 사전점검을 믿은 에이전트가
  자금 이동 직전에 거부당합니다. 수량은 어떤 체크도 커버하지 않고 있었습니다
  (매수는 `est_cost=0`이라 `ord_alow_amt >= 0`이, 매도는 `held >= 0`이 공허하게
  참이 됐습니다). 출력 페이로드에 `checks.qty_ok` 키가 추가됩니다.
- **0D(주식호가잔량) 이벤트의 모양이 크게 바뀝니다.** `ts`가 항상 `null`이던
  것이 채워지고, 필드 ID 41~80이 숫자 ID 문자열 대신
  `ask*`/`bid*`/`ask_qty*`/`bid_qty*` 정규명으로 나옵니다. 매도호가는
  방향지시자 부호를 잃습니다(가격 41~60은 `_ABS_FIELDS`, 수량 61~80은
  `_SIGNED_FIELDS`). **NDJSON 소비자에게 breaking이고, 이미 녹화해 둔 0D 파일의
  해석도 함께 바뀝니다.**
- **sqlite export에 `UNIQUE(ts, symbol, type, raw_json)` 제약이 생기고, 기존
  파일이 자동 마이그레이션됩니다.** sqlite는 csv/parquet과 달리 append인데 PK도
  UNIQUE도 없이 무조건 INSERT해서, 같은 구간을 다시 내보내면 행이 그대로
  배가 됐습니다. `CREATE TABLE IF NOT EXISTS`는 기존 테이블에 새 제약을 붙여
  주지 않으므로 구버전 파일은 스키마를 승격합니다(rename → 새 DDL →
  `INSERT OR IGNORE SELECT` → drop). **이때 이미 쌓여 있던 중복 행이 한 건으로
  접힙니다.** 옛 export 파일의 행 수가 달라집니다. `price`/`volume`을 제약에
  넣지 않은 것은 SQL의 `NULL != NULL` 때문에 둘 다 NULL인 0D 행에서 제약이
  무력해지기 때문입니다.
- **`stream`·`watch`의 실패 경로 종료 코드가 바뀝니다.** 종전에 0을 반환하던
  경로들입니다: 인증 실패와 토큰 없음이 3, 등록 실패·연결 거부·예상 외 예외·
  비정상 종료가 2입니다. `$?`로 분기하는 스크립트나 에이전트는 다른 값을
  보게 됩니다. 종전에는 table 모드가 연결 거부와 예상 외 예외에서도 전부 0을
  반환해 실패가 성공으로 보였습니다. 서버의 정상 종료는 그대로 0입니다.
  **`-f json`/`-f csv`로 `watch`를 부르면 이제 거부됩니다(exit 1).** Rich TUI라
  NDJSON 계약을 지킬 수 없는데 조용히 TUI를 띄우고 stdout을 비워 뒀습니다.
  REST 조회 전에 거부합니다.
- **`auth logout`이 `KIWOOM_TOKEN`에서 온 토큰을 폐기했을 때는 키체인 토큰을
  지우지 않습니다.** 키체인 항목은 그것이 방금 폐기한 바로 그 토큰일 때만
  지웁니다(env와 키체인 값이 같으면 죽은 토큰을 남기지 않기 위해 지웁니다).
  `revoke_token`이 `{token_source, keychain_token_deleted}`를 돌려주고 json 모드
  envelope에도 두 필드가 실립니다.
- **캐시 두 곳이 업그레이드 후 첫 실행에서 다시 조회합니다.** 미국 거래소
  캐시는 도메인별로 분리되고 24시간 TTL이 붙습니다 — **v2.12 이하의 평문
  형식은 마이그레이션하지 않고 버립니다.** 시각이 없어 신선도를 알 수 없고
  어느 도메인에서 학습했는지도 모르기 때문입니다. 종목 리스트 캐시는 7일 TTL이
  붙어, `fetched_at`이 없거나 해석 불가면 만료로 취급합니다.

**Fixed (후속 리뷰에서 추가로 발견)**

- **`kiwoom stream *`이 등록 ack 없이 끝난 스트림을 성공으로 보고했습니다.**
  위의 등록 **실패 응답**(`return_code != 0`) 처리는 이번 릴리스에서 고쳤지만,
  ack가 아예 오지 않는 경우가 남아 있었습니다. LOGIN 성공 → REG 전송 → 서버가
  답 없이 종료하면 수신 루프가 그냥 빠져나와 exit 0이 나갔습니다. json 모드는
  envelope조차 없이 빈 stdout으로 끝나서, 에이전트 입장에서는 "성공, 이벤트
  0건"과 구별할 방법이 없었습니다 — 구독은 한 번도 성사되지 않았는데도요.
  이제 등록 ack 수신 여부를 추적해, 루프가 어떤 이유로 끝나든(소켓 정상 종료,
  `--duration`/`--until` 만료, PING 에코 중 종료) ack가 없었으면 exit 2 +
  `UPSTREAM_ERROR`로 끝냅니다. ack를 받은 뒤의 정상 종료는 그대로 exit 0입니다.
- **WebSocket 필드 `9001`(종목코드)이 미국 티커와 금현물 코드를 훼손했습니다.**
  선행 영문자를 무조건 벗기고 있어서 `NVDA` → `VDA`, `TSLA` → `SLA`, `F` → `""`,
  `M04020000` → `04020000`이 됐습니다. 국내 6자리 코드에서만 우연히 맞는
  규칙이었습니다. REST 경로(`stk_cd`)는 이미 "영문 1자 + 숫자 6자리" 모양에만
  적용하는 `strip_kr_market_prefix`로 고쳐져 있었고 WS 경로만 옛 패턴이 남아
  있었습니다. 이제 두 경로가 같은 헬퍼를 씁니다.
- **`dashboard`의 거래량 상위가 `market rank volume`과 다른 목록을
  보여줬습니다.** 두 명령의 ka10030 body는 `mang_stk_incls` 하나를 빼면 완전히
  동일했는데, `dashboard`만 `"1"`(관리종목 미포함)을 하드코딩하고 있었습니다.
  관리종목은 상장폐지·자본잠식·감사의견 이슈로 거래량이 튀는 날 상위권에
  들어오는 종목이라, 같은 "당일 거래량 상위"가 두 명령에서 갈렸습니다.
  `market rank volume`의 기본값(`"0"`, 관리종목 포함)에 맞췄습니다.
  ka10030의 이 필드는 boolean이 아니라 15개 값짜리 종목조건 코드북입니다.

**알려진 한계 (이번에 해결하지 못함)**

- **실시간 필드 `21`(호가시각)에 `+09:00`이 무조건 붙습니다.** 미국 실시간
  타입(FT)이 0D와 **같은 필드 ID 체계**를 쓰기 때문에, 이번 0D 정규화가 US
  이벤트에도 KST 주장을 함께 확산시켰습니다. 미국 실시간 시각이 KST인지에
  대해 키움의 소스들이 서로 어긋나고 워크북 예시는 합성이라 판별할 수
  없었습니다. 검증하지 못한 것이지 확인한 것이 아닙니다. 틀렸다면 13~14시간
  오차입니다. `kiwoom stream *`은 국내 소켓(`/api/dostk/websocket`)만 물고 미국
  실시간은 `/api/us/websocket`이라 오늘 이 경로에 도달할 방법은 없지만,
  녹화된 FT 프레임을 `history`로 읽는 소비자는 그 `ts`를 KST로 믿으면 안
  됩니다. 실제 US 프레임 한 개면 결판납니다. 같은 경고를 `normalize.py`와
  AGENTS.md 양쪽에 적어 뒀습니다.

**Internal**

- 코드북 드리프트 테스트에 술어 3(부분 겹침, 양쪽 배타 키)을 추가해 세 술어를
  전부 양방향으로 계산합니다. 해저드 쌍은 352개입니다(이전에 보고된 356은
  합집합도 그 보고서 자체의 산술도 아니었습니다). **해저드 쌍을 실제로 병합해
  보는 sweep에서, 값이 실제로 달라지는 병합 54개가 기존 스위트를 전부 통과하는
  것이 드러났습니다.** 개별 값 고정 54개를 새로 쓰는 대신 "이름이 다른 두
  코드북이 동치가 되면 실패"라는 불변식으로 비동일 352쌍을 구성적으로
  덮었습니다. 첫 하네스가 "476/476 적발"을 보고했던 것은 **하네스가 깨졌다는
  증거**였습니다 — 값이 원래 같은 120쌍은 원리상 병합해도 동작이 바뀌지 않아
  100% 적발이 나올 수 없습니다.
- `str→dict` 중첩 코드북(`PROGRAM_MARKET_BY_EXCHANGE`)이 드리프트 수집기에
  보이지 않던 것을 명시적 인벤토리로 고정했습니다. 수집기를 재귀시키는 대신
  인벤토리를 택한 것은, 내부 dict의 부분집합 관계와 외부 상수의 병합 해저드가
  같은 축에서 셀 수 있는 질문이 아니기 때문입니다.
- `config prune-ledger`의 원장 잠금을 논블로킹으로 바꿨습니다. 종전에는 잠금
  경합 시 안내 문구가 뜰 기회 없이 그냥 매달렸습니다(20초 kill을 넘겨 재현).
  주문 경로의 잠금은 그대로 블로킹입니다 — 동시 주문이 `LEDGER_BUSY`로
  펄럭이면 안 됩니다.
- WebSocket 핸드셰이크의 전송 페이로드(LOGIN 토큰, REG의 타입·종목·grp_no·
  refresh)를 `stream`·`watch` 양쪽에서 고정했습니다. 종전 스위트는 세 변이를
  동시에 걸어도 통과했습니다.
- **자기참조 `parametrize` 8건을 리터럴로 고정했습니다.**
  `parametrize(list(MAP.items()))`는 기대값을 검증 대상 상수에서 가져오므로
  상수를 어떻게 바꿔도 자기모순이 없어 항상 통과합니다. 23개 상수 29곳을
  전수 조사했습니다. **스윕 방법론이 결론을 뒤집었다는 점을 남겨 둡니다** —
  각 코드북의 *앞* 두 항목을 맞바꾸면 24/24 적발(구멍 0건)이 나오는데, 첫 키가
  대개 CLI 기본값이고 기본값은 다른 테스트가 이미 고정하고 있어서 자기참조
  구멍이 아니라 기본값 커버리지를 측정하기 때문입니다. *뒤* 두 항목(비기본값)을
  맞바꾸면 같은 스위트에서 구멍 7건이 드러났고, `_constants.py` 밖에 있어
  스윕이 건너뛴 `ORDER_TYPES`까지 별도 측정해 총 8건이었습니다. 그중
  `ORDER_TYPES`의 `ioc`↔`fok`(체결 계약이 다릅니다), `ETF_ALL_TAXABLE`의
  과세↔비과세, `ELW_RIGHT_TYPE_1DIGIT`의 콜↔풋처럼 **양쪽 모두 스펙상 유효한
  코드로 남는 맞바꿈**은 서버가 200에 정상 응답을 주므로 어디에도 오류가
  남지 않습니다. 고정 후 같은 변이 9개가 전부, 맞바꾼 키에 해당하는 파라미터
  2건씩만 정확히 실패합니다.
- **`dashboard` ↔ `market rank volume`의 ka10030 body를 전 필드 동일로
  단언합니다.** 종전에는 `mang_stk_incls`만 예외로 두고 나머지를 비교했는데,
  예외 키를 하나 허용하면 다음 드리프트가 "원래 다르던 것"에 섞여 보이지
  않습니다.
